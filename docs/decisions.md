...
2. **固定项 + 临时项组合**：应用资源按 `ApplicationItemIdentity` 组合：优先使用实际 `.app` bundle 路径，同路径的多个运行实例聚合为一个 item；不同路径即使 bundle identifier 相同也生成不同 item。固定项相对顺序持久化，临时项由运行资源产生；手动调整的混合排位属于本次会话，不隐式固定临时项。固定项的重排锚点不越过临时区（临时项恒居最右）：固定项落到临时区即收敛到固定项末位，不产生会话级尾部位。普通应用退出后临时项消失，固定项保留；临时项手动移除仅隐藏当前资源路径的运行实例，新实例或重启 TideBar 后恢复。Finder 沿用「Finder 角色」的逻辑运行规则。
...
1. **身份分层**：`AppIdentity` 是运行策略身份，负责 bundle identifier 规范化、Finder 特判、窗口与进程行为；`ApplicationItemIdentity` 是栏内资源身份，优先由 `.app` bundle 的规范化实际路径构成，bundle-only 仅作为旧数据或无法定位时的降级。启动、窗口操作与文件定位使用采集阶段解析出的 URL、PID 和运行实例，不用 bundle-only 字符串反查一个不确定的系统对象。
2. **应用聚合、窗口归属到进程**：同一资源路径的多个运行实例在图标栏只生成一个条目；同 bundle identifier 的不同 `.app` 路径各自生成条目。窗口知识按 PID 观测并聚合，每个窗口保留 owner PID，窗口动作始终发送给所属实例。任一实例无法完整确认窗口集合或最小化态时，聚合结果为未知，不报告不完整点数。
...
4. **身份对账双通道**：动作校验（AppActionDispatcher）和窗口观测（WindowStore）对 bundle 进程按 `AppIdentity` 比对，对裸进程按可执行路径比对；条目选择、固定去重、拖拽投递和隐藏状态按 `ApplicationItemIdentity` 定位资源。
5. **identity 与内容修订分离**：资源 item identity 只负责复用对应路径的视图；运行策略、进程、能力或可观测窗口模型变化必须替换最新模型。被跟踪条目消失、窗口知识降级或可观测窗口模型变化时，已展开潮涌立即失效并关闭。

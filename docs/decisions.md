...
3. **应用能力不泛化为所有条目的必填字段**：文件、目录通过 NSWorkspace 打开，拥有自身菜单，不提供应用退出或运行短线；打开文件不把它合并进默认打开应用。潮涌体由类型自己提供（应用 = 窗口列表，目录 = 最近文件，见「潮涌」）；小组件可接入自身行为，不要求具备 URL、bundle identifier 或进程。小工具实例的存在性与顺序直接来自 TideBar item（`kind == widget`），不建立独立 widget 列表或存储；顺序与其他条目同走栏内拖拽重排，实例的添加与删除仅经设置页。

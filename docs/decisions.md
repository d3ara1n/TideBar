...
2. **固定项 + 临时项组合**：应用按既有 AppIdentity 合并，已固定且运行的应用只有一个 item，窗口点、潮涌、角标、激活与退出行为不受固定状态影响。固定项相对顺序持久化，临时项由运行应用产生；手动调整的混合排位属于本次会话，不隐式固定临时项。普通应用退出后临时项消失，固定项保留；临时项手动移除仅隐藏当前运行实例，新实例或重启 TideBar 后恢复。Finder 沿用「Finder 角色」的逻辑运行规则。

...
3. **应用能力不泛化为所有条目的必填字段**：文件、目录通过 NSWorkspace 打开，拥有自身菜单，不提供应用退出或运行短线；打开文件不把它合并进默认打开应用。潮涌体由类型自己提供（应用 = 窗口列表，目录 = 最近文件，见「潮涌」）；小组件等未来类型可接入自身行为，不要求具备 URL、bundle identifier 或进程。
...
5. **引用存取与行为分发分离**：`ItemReference` 是 schema 与 payload 构成的不透明引用，解释权属于操作。固定记录存取、列表组合、关联创建和类型行为各自收敛到 module；栏与设置页共用创建、去重和存储入口。应用类型复用现有窗口与进程 module，不建设通用插件框架或提前实现小组件。

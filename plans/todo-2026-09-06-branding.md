...
> 状态：README（双语）、LICENSE、命名统一、关于页已完成；话术源 copy.md 为占位初稿、待人工审核定稿；品牌素材已导出，应用内 PDF 已接入（菜单栏／关于页／引导页），README／网站标记与正式 `.app` 图标待接入；Dock 图标由用户通过 Icon Composer 制作；演示素材待录屏。本地化升格为发布前置（App UI 层由 todo-2026-09-05-localization.md 承接）。
...
对外话术源头已建立（占位初稿）：[docs/copy.md](../docs/copy.md)（slogan、pitch、功能四条、权限话术、FAQ、对比表、载体派生规则；内容系补材料时 AI 生成，未经人工审核）。本立项的 README、关于页工作以它为输入。

## 范围

### 0. 对外话术源（结构已建，内容为占位初稿）

- [docs/copy.md](../docs/copy.md)：定位是对外文案唯一源头（README、关于页、Releases 说明、官网从它派生，文案变更先改该档再同步载体）；现有内容系补材料时 AI 生成，**未经人工审核，审核定稿是本立项未尽事项**，定稿前不作为话术真值。

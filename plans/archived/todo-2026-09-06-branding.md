...
> 状态：已完成。双语 README、官网文案、产品词汇表、LICENSE、应用「关于」页与品牌素材全部交付，人工验收通过。
...
- [x] 人工确认菜单栏图标的显示效果与 Icon Composer 素材导入。
- [x] 录制真实操作并接入应用与官网：onboarding 第 1 步循环演示视频、网站汐线导航「演示」窗口（随显隐暂停，reduced-motion 不自动播放）。
- [x] README 重写与演示素材接入：由 `todo-2026-09-16-readme-rewrite.md` 承接（发布前置）。
- [x] 仓库公开时检查下载、官网与 GitHub 社交预览是否可访问；官网目前使用 GitHub 仓库预览图，可按需要上传专用分享图。
